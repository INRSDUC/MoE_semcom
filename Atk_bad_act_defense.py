...
parser.add_argument('--checkpoint-path', default='/home/necphy/ducjunior/BERT_Backdoor/checkpoints/deepsc_AWGN_JSSC_type2_MOD_nochoke_def', type=str)
parser.add_argument('--loadcheckpoint-path', default='/home/necphy/ducjunior/BERT_Backdoor/checkpoints/deepsc_AWGN_JSSC_type2_MOD_nochoke', type=str)
...
if __name__ == '__main__':
    ...
    poisoned_list = ds["train"]#poison_dataset(train_dataset,trigger_token='cf', poison_ratio=0.1)
    ...
    model_path = "/home/necphy/ducjunior/BERT_Backdoor/checkpoints/deepsc_AWGN_JSSC_type2_MOD_nochoke/checkpoint_full05.pth"#model_paths[-1]  # Load the latest checkpoint
    ...
    for epoch in range(args.epochs):
        # 1) Standard train + validate
        train_loss = train(epoch, args, train_dataset=combined_dataset, net=deepsc, criterion=criterion, opt=optimizer)
        val_loss, val_acc, val_prec, val_rec, val_f1, val_rate = validate(epoch, args, deepsc, test_eur=test_eur)

        # 2) Poison‐ASR eval on the *current* weights
        #    (you could subsample a small poison set to save time)
        poison_list = poison_dataset(ds["validation"], trigger_token="cf",  poison_ratio=0.1)
        poison_ds   = Dataset.from_list(poison_list).map(preprocess_sst2, batched=True)
        poison_ds.set_format(type="torch",
            columns=["input_ids","attention_mask","label","is_poisoned","original_label"])
        asr_cf = evaluate_attack_success_rate(model=deepsc, poisoned_dataset=poison_ds, batch_size=args.batch_size, n_var=0.1)

        poison_list2 = poison_dataset(ds["validation"], trigger_token="ab", poison_ratio=0.1)
        poison_ds2    = Dataset.from_list(poison_list2).map(preprocess_sst2, batched=True)
        poison_ds2.set_format(type="torch",
            columns=["input_ids","attention_mask","label","is_poisoned","original_label"])
        asr_ab = evaluate_attack_success_rate(model=deepsc, poisoned_dataset=poison_ds2, batch_size=args.batch_size, n_var=0.1)

        # 3) (Optional) save checkpoint
        if not os.path.exists(args.loadcheckpoint_path):
            os.makedirs(args.loadcheckpoint_path)
        torch.save(deepsc.state_dict(),
            os.path.join(args.loadcheckpoint_path, f'checkpoint_full{epoch+1:02d}.pth'))

        print(f"Epoch {epoch+1}: "
            f"Train L={train_loss:.4f}, Val Acc={val_acc:.3f}, "
            f"ASR_cf={asr_cf:.2f}%, ASR_ab={asr_ab:.2f}%")
