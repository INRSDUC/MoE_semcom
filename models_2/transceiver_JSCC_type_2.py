...
class JSCC_DeepSC(nn.Module):
    def __init__(self, num_layers, d_model, num_heads, dff, num_classes ,freeze_bert, dropout=0.9):
        ...
        self.channel_decoder = ChannelDecoder(128, d_model, 512,d_model)
    ...
    def forward(self, input_ids, attention_mask, channel_type, n_var):
        ...
        y       = y[:, 0, :]    # [B, D]
        # print("y",y.shape)
        # 2. Hyperprior: encode side-latent
        z = self.hyper_encoder(y)  # [B, D]
        # z_tilde = z 
        if self.training:
            z_tilde = z + torch.rand_like(z) - 0.5
        else:
            z_tilde = torch.round(z)
        # 3. Hyper-decoder: predict Gaussian params for y
        params = self.hyper_decoder(z_tilde)
        # print('params',params.shape)
        mu, raw_scale = params.chunk(2, dim=1)  # each [B, D]
        sigma = F.softplus(raw_scale)
        
        # print(f"  μ : mean={mu.mean():.3f}, std={mu.std():.3f}")
        # print(f"  σ : mean={sigma.mean():.3f}, min={sigma.min():.3f}, max={sigma.max():.3f}")
        #by passing bit quantization
        
        
        # y_tilde = y
        # 4. Quantize y (surrogate during training)
        if self.training:
            y_tilde = y + torch.rand_like(y) - 0.5
        else:
            y_tilde = torch.round(y)
        # # 5. Compute entropy (rate) terms
        p_y = discrete_probability(y_tilde, mu, sigma)
        B_y = -torch.log2(p_y).sum(dim=1)  # bits for y
        # simple standard-normal prior on z
        p_z = discrete_probability(z_tilde,
                                   torch.zeros_like(z_tilde),
                                   torch.ones_like(z_tilde))
        B_z = -torch.log2(p_z).sum(dim=1)
        # print("B_y",B_y.shape)
        # print("B_z",B_z.shape)
        rate_loss = (B_y + B_z).mean()
        # 6. Channel encoding & normalization
        channel_enc_output = self.channel_encoder(y_tilde)
        ...
        # Return both prediction and rate loss for joint training
    #     print("y:", y.shape,
    #   "y_tilde:", y_tilde.shape,
    #   "params:", params.shape,
    #   "Tx_sig:", Tx_sig.shape,
    #   "pred_logits:", pred_logits.shape)
        return pred_logits, rate_loss
