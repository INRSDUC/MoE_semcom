...
class ModulationBlock(nn.Module):
    def __init__(self, modulation_order=16):
    ...
    def _generate_constellation(self):
        ...
        M = int(self.modulation_order**0.5)
        real_parts = torch.linspace(-M+1, M-1, M)
        imag_parts = torch.linspace(-M+1, M-1, M)
        constellation = torch.cartesian_prod(real_parts, imag_parts)
        constellation = constellation[:, 0] + 1j * constellation[:, 1]
        constellation /= constellation.abs().max()  # Normalize
        return constellation

    def forward(self, x):
        # Flatten the input and map to constellation points
        batch_size, seq_len, feat_dim = x.shape
        x = x.view(-1, feat_dim)  # Flatten
        x = torch.tanh(x)  # Ensure values are between -1 and 1
        x = (x + 1) / 2  # Map to [0, 1]
        x = (x * (self.modulation_order - 1)).round().long()  # Map to indices
        x = self.constellation[x]  # Map to constellation points
...
class NormalDeepSC(nn.Module):
    def __init__(self, num_layers, d_model, num_heads, dff, num_classes ,freeze_bert, dropout=0.9, modulation_order = 32):
    ...
    def forward(self, input_ids, attention_mask,  n_var ,channel_type= 'AWGN'):
        ...
        # modulated_signal = self.modulation(Tx_sig)
