...
parser.add_argument('--checkpoint-path', default='/home/necphy/ducjunior/BERTDeepSC/checkpoints/deepsc_AWGN_nodecoder_SNR_Rician/checkpoint_full16.pth', type=str)
...
def fine_prune_model(model, amount=0.2):
    """
...
def finetuning_pruning(model_dir, finetuning_data, pruning_amount ):
    # process fine-tuning data
        ...
        if finetuning_data == "/home/necphy/ducjunior/BERTDeepSC/BackdoorPTM_main/Datasets/imdb/train.tsv":
            ...
            df_val = df_val.sample(15000, random_state=2025)
        ...
        FTPPT.load_state_dict(torch.load("/home/necphy/ducjunior/BERTDeepSC/poisoned_encoder_channel_type_1_v9_nodecoder_Rician.pth", map_location=device), strict=False)
        print('Model loaded!')
        FTPPT.to(device)
    ...
    # fine-tuning
        ...
        noise_std = np.random.uniform(SNR_to_noise(0), SNR_to_noise(00), size=(1))
...
if __name__ == '__main__':
    ...
    for pruning_amount in pruning_set:
        ...
        finetuned_PTM = finetuning_pruning(model_dir, finetuning_data, pruning_amount=pruning_amount)
