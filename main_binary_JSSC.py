    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            loss, accuracy, precision, recall, f1, rate_loss = val_step_with_smart_simple_JSCC(
                net, labels, criterion, input_ids, attention_mask, channel=args.channel, n_var=noise_std.item()
            )
...
# def train(epoch, args, net,criterion, opt, mi_net=None):
    ...
    for batch in pbar:
        ...
        loss,ori_loss, smooth_loss, rate_loss = train_step_with_smart_simple_JSCC(
        net, labels, opt, criterion,
        inputs["input_ids"].to(device),
        inputs["attention_mask"].to(device),
        channel=args.channel,
        n_var=noise_std.item(),
        alpha=args.alpha,
        lambda_rate=args.lambda_rate)
        total_loss = total_loss+  loss
        pbar.set_description(f'Epoch: {epoch + 1}; Type: Train; Loss: {loss:.5f},ori_loss: {ori_loss:.5f},smooth_loss: {smooth_loss:.5f},rate_loss: {rate_loss:.5f}')
...
if __name__ == '__main__':
    ...
    for name, module in deepsc.named_modules():
        if any(p in name for p in ('hyper_encoder', 'hyper_decoder')): 
            module.apply(initialize_weights)
