...
import numpy as np
from utils_oke import SNR_to_noise, val_step_with_smart_simple_JSCC,  train_step_modulated_budget, train_step_modulated_adv, train_step_router_budget_v3, train_step_router_budget_v4, train_step_faithful, train_step_router_budget_v5, train_step_router_budget_v6
...
def train(epoch, args, train_dataset, net, criterion, opt, mi_net=None):
    from torch.utils.data import DataLoader
    import numpy as np
    pbar = tqdm(DataLoader(train_dataset, batch_size=args.batch_size, num_workers=0,
                           pin_memory=True, shuffle=False))
    total_loss = 0.0
    net.train()
